...
class TMSi_GUI:
    def __init__(self, parent):
        ...
        self.lbl0 = ttk.Label(self.parent.frame_tmsi, text='Label for TMSi 1 (Lower serial number)')
        # self.lbl0.pack(fill='x', expand=True)
        self.t0 = tk.Entry(self.parent.frame_tmsi, textvariable=self.tmsi_1_name)
        self.t0.insert(0, "FLX")
        # self.t0.pack(fill='x', expand=True)
        self.t0.focus()
        self.lbl0.place(x=10, y = 30)
        self.t0.place(x=260, y = 30)
        

        self.lbl1 = ttk.Label(self.parent.frame_tmsi, text='Label for TMSi 2 (Higher serial number)')
        # self.lbl1.pack(fill='x', expand=True)
        self.t1 = tk.Entry(self.parent.frame_tmsi, textvariable=self.tmsi_2_name)
        self.t1.insert(0, "None")
        # self.t1.pack(fill='x', expand=True)
        self.t1.focus()
        self.lbl1.place(x=10, y=55)
        self.t1.place(x=260, y=55)


        self.dev_warn = ttk.Label(self.parent.frame_tmsi, text='NOTE: Use "None" to not include tmsi (prefer TMSi 1)')
        # self.dev_warn.pack(fill='x', expand=True)
        self.dev_warn.place(x=400, y=30)

        self.tmsi_activate_button = tk.Button(self.parent.frame_tmsi, text='Start Devices', bg ='yellow')
        self.tmsi_activate_button['command'] = self.init_tmsi
        # self.tmsi_activate_button.pack()
        self.tmsi_activate_button.place(x=400, y=55)
        ...
        self.channel_warn = ttk.Label(self.parent.frame_tmsi, text='NOTE: Enter channel number before checking the box || Use "None" to not include channel')
        # self.channel_warn.pack(fill='x', expand=True)
        self.channel_warn.place(x=10, y=90)

        self.c1 = tk.Checkbutton(self.parent.frame_tmsi, text='GRID',variable=self.grid_check, onvalue=1, offvalue=0, command= self.set_grid)
        # self.c1.pack()
        self.c1.place(x=300, y=110)
        self.grid_check.set(1)

        self.c2 = tk.Checkbutton(self.parent.frame_tmsi, text='BIP',variable=self.BIP_check, onvalue=1, offvalue=0, command=self.set_bip)
        # self.c2.pack()
        self.c2.place(x=300, y=130)
        self.BIP_check.set(1)

        self.c3 = tk.Checkbutton(self.parent.frame_tmsi, text='AUX',variable=self.AUX_check, onvalue=1, offvalue=0, command=self.set_aux)
        # self.c3.pack()
        self.c3.place(x=300, y=150)
        self.AUX_check.set(1)

        self.c4 = tk.Checkbutton(self.parent.frame_tmsi, text='Sample rate',variable=self.fs_check, onvalue=1, offvalue=0, command=self.set_fs)
        # self.c4.pack()
        self.c4.place(x=300, y=170)
        self.fs_check.set(1)

        self.c5 = tk.Checkbutton(self.parent.frame_tmsi, text='Triggers',variable=self.trig_check, onvalue=1, offvalue=0, command= self.set_trig)
        # self.c5.pack()
        self.c5.place(x=300, y=190)
        self.trig_check.set(1)


        self.grid_num = tk.StringVar()
        self.lbl2 = ttk.Label(self.parent.frame_tmsi, text='Grid Channels')
        # self.lbl2.pack(fill='x', expand=True)
        self.t2 = tk.Entry(self.parent.frame_tmsi, textvariable=self.grid_num)
        self.t2.insert(0, "64")
        # self.t2.pack(fill='x', expand=True)
        self.t2.focus()
        self.lbl2.place(x=10, y=113)
        self.t2.place(x=150, y=113)

        self.bip_num = tk.StringVar()
        self.lbl3 = ttk.Label(self.parent.frame_tmsi, text='BIP Channels')
        # self.lbl3.pack(fill='x', expand=True)
        self.t3 = tk.Entry(self.parent.frame_tmsi, textvariable=self.bip_num)
        self.t3.insert(0, "4")
        # self.t3.pack(fill='x', expand=True)
        self.t3.focus()
        self.lbl3.place(x=10, y=133)
        self.t3.place(x=150, y=133)

        self.aux_num = tk.StringVar()
        self.lbl4 = ttk.Label(self.parent.frame_tmsi, text='AUX Channels')
        # self.lbl4.pack(fill='x', expand=True)
        self.t4 = tk.Entry(self.parent.frame_tmsi, textvariable=self.aux_num)
        self.t4.insert(0, "4")
        # self.t4.pack(fill='x', expand=True)
        self.t4.focus()
        self.lbl4.place(x=10, y=153)
        self.t4.place(x=150, y=153)

        self.fs_num = tk.StringVar()
        self.lbl5 = ttk.Label(self.parent.frame_tmsi, text='Sampling rate')
        # self.lbl5.pack(fill='x', expand=True)
        self.t5 = tk.Entry(self.parent.frame_tmsi, textvariable=self.fs_num)
        self.t5.insert(0, "2000")
        # self.t5.pack(fill='x', expand=True)
        self.t5.focus()
        self.lbl5.place(x=10, y=173)
        self.t5.place(x=150, y=173)

        
        self.trig_num = tk.StringVar()
        self.lbl6 = ttk.Label(self.parent.frame_tmsi, text='Triggers (0: Off, 1: On)')
        # self.lbl6.pack(fill='x', expand=True)
        self.t6 = tk.Entry(self.parent.frame_tmsi, textvariable=self.trig_num)
        self.t6.insert(0, "1")
        # self.t6.pack(fill='x', expand=True)
        self.t6.focus()
        self.lbl6.place(x=10, y=193)
        self.t6.place(x=150, y=193)

        self.tmsi_configure_button = tk.Button(self.parent.frame_tmsi, text='Configure Channels', bg ='yellow')
        self.tmsi_configure_button['command'] = self.config_chan
        # self.tmsi_configure_button.pack()
        self.tmsi_configure_button.place(x=10, y=215)


        self.imp_plot_button = tk.Button(self.parent.frame_tmsi, text='Get Imp Plot', bg ='yellow')
        self.imp_plot_button['command'] = self.imp_plot
        # self.imp_plot_button.pack()
        self.imp_plot_button.place(x=10, y=240)

        self.vis_sig_button = tk.Button(self.parent.frame_tmsi, text='Visualize signals', bg ='yellow')
        self.vis_sig_button['command'] = self.start_plotter
        # self.vis_sig_button.pack()
        self.vis_sig_button.place(x=100, y=240)

        self.vis_heatmap_button = tk.Button(self.parent.frame_tmsi, text='Visualize heatmap', bg ='yellow')
        self.vis_heatmap_button['command'] = self.start_heatmap
        # self.vis_heatmap_button.pack()
        self.vis_heatmap_button.place(x=210, y=240)

        self.start_dump_button = tk.Button(self.parent.frame_tmsi, text='PUSH TMSi', bg = 'yellow')
    ...
    def push_tmsi(self):
        self.parent.tmsi_dev = self.device_dict
        ...
        showinfo(title='Information', message='Created device dict')
        self.start_dump_button.config(bg = 'green')
